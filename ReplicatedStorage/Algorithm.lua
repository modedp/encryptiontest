...
return function(Message,KeyData)
	...
	local data = {
		...
		["shared"] = KeyData.shared,
		private = KeyData.private
	}
	if data.public then
		local res = bytesToNumber(stringToBytes(Message), unpack(defaultSettings))
		local encrypted = crypt(data, res)
		if debug_ then
			print("Took " .. os.clock() - startTime .. " seconds to encrypt.")
		end
		return encrypted
	elseif data.private then
		local decrypted = crypt(data, Message)
		local decryptedBytes = numberToBytes(decrypted, unpack(defaultSettings))
		if debug_ then
			print("Took " .. os.clock() - startTime .. " seconds to decrypt.")
		end
		return bytesToString(decryptedBytes)
	end
	error("[CODE] Script did not input a key for algorithm!")
end
